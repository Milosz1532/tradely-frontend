...
import { fetchUserActiveAnnouncements } from '../../services/Api'
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'

import {
	RectangularActiveAnnouncement,
	RectangularActiveAnnouncementLoading,
} from '../../components/Announcements/RectangularActiveAnnouncement'
import Button from '../../components/Layout/Button'

export default function ProfileActiveAnnouncements() {
	...
	useEffect(() => {
		const getUserAnnouncements = async () => {
			try {
				const announcementsData = await fetchUserActiveAnnouncements(currentPage)
	...
	return (
		<>
			<div className='main-content-header pb-3'>
				<h5 className='m-0 ms-2'>Moje aktywne ogłoszenia</h5>
			</div>
			...
			<div className='row px-2'>
				{isLoading ? (
					...
					<div className='text-center mt-4'>
						<p className='color-gray'>W tej chwili nie masz żadnych aktywnych ogłoszeń.</p>
						<p className='color-gray'>Dodaj swoje ogłoszenie już teraz</p>
						<Link to={'/createAnnouncement'}>
							<Button text={'Dodaj ogłoszenie'} color={true} rounded={true} />
						</Link>
					</div>
				)}
			</div>
